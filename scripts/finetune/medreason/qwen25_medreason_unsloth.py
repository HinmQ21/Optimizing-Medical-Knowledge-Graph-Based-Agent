...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "MedReason-style SFT for Qwen2.5 with Unsloth + Transformers Trainer. "
            "This keeps assistant-only masked loss like the original MedReason SFT pipeline, "
            "but removes accelerate/DeepSpeed."
        )
    )
    parser.add_argument(
        "--model-path",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Used only when --eval-split is not provided. Set 0 to disable eval split creation.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="outputs/medreason_qwen25_3b_unsloth",
    )
    ...
    parser.add_argument("--logging-steps", type=int, default=3)
    ...
    parser.add_argument("--run-name", default="medreason-qwen25-3b-unsloth")
    ...
    parser.add_argument(
        ...
        default=True,
        help="Use 4-bit base weights with LoRA adapters.",
    )
    parser.add_argument(
        "--dtype",
        default=None,
        choices=[None, "float16", "bfloat16"],
        help="Override model dtype for Unsloth loading.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Log token-level train accuracy like the original MedReason SFT script. "
            "This enables UNSLOTH_RETURN_LOGITS automatically."
        ),
    )
    parser.add_argument(
        "--thinking-section-title",
        default="Thinking",
        help="Section title used before the reasoning chain.",
    )
    parser.add_argument(
        "--final-section-title",
        default="Final Response",
        help="Section title used before the final answer.",
    )
    parser.add_argument(
        "--chat-template-style",
        default="auto",
        choices=["auto", "qwen"],
        help="Fallback style when tokenizer.chat_template is missing.",
    )
...
def format_completion(example: dict[str, Any], args: argparse.Namespace) -> str:
    ...
    return (
        f"## {args.thinking_section_title}\n\n"
        f"{reasoning}\n\n"
        f"## {args.final_section_title}\n\n"
        f"{response}"
    )
...
def normalize_special_tokens(tokenizer, model, args: argparse.Namespace) -> None:
    ...
    if not getattr(tokenizer, "chat_template", None):
        if args.chat_template_style in {"auto", "qwen"}:
            tokenizer.chat_template = DEFAULT_QWEN_TEMPLATE
        else:
            raise ValueError(f"Unsupported chat template style: {args.chat_template_style}")
...
def build_model_and_tokenizer(args: argparse.Namespace):
    ...
    normalize_special_tokens(tokenizer, model, args)
...
def render_chat(prompt: str, completion: str, tokenizer) -> tuple[str, str]:
    ...
    if getattr(tokenizer, "chat_template", None):
        ...
        prompt_text = f"<|im_start|>user\n\n{prompt}<|eot_id|><|im_start|>assistant\n\n"
        full_text = f"{prompt_text}{completion}<|eot_id|>"
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self._log_train_acc and model.training and "labels" in inputs:
            ...
            if logits is not None:
                self._running_token_stats.update(
                    logits,
...
def build_run_config(
    ...
    return {
        **vars(args),
        ...
        "trainer_type": "transformers.Trainer + unsloth.FastLanguageModel",
        "dataset_format": "chat_with_assistant_only_masked_labels",
        "loss_behavior": "assistant_only_via_-100_labels",
        "truncation_behavior": "left_truncate_to_match_original_medreason_sft",
