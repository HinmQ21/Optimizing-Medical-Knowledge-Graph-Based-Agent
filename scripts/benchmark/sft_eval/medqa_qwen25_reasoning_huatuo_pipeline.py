...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run MedQA/MedMCQA benchmark for Qwen2.5 reasoning-SFT models using a thinking-style prompt."
    )
    parser.add_argument(
        "--dataset",
        default="medqa",
...
_DATASET_DEFAULTS = {
    "medqa":   {"path": "dataset/MedQA",           "split": "test",       "output_dir": "results/medqa_qwen25_reasoning_pipeline"},
    "medmcqa": {"path": "dataset/MedMCQA_4options", "split": "validation", "output_dir": "results/medmcqa_qwen25_reasoning_pipeline"},
}
...
def main() -> None:
    ...
    run_tag = f"{args.dataset}_qwen25_reasoning"
    if args.tag:
        run_tag = f"{run_tag}_{args.tag}"

    print(f"\n=== Running {model_path.name} on {args.dataset.upper()} ({split}) with Qwen2.5 reasoning prompt ===")
