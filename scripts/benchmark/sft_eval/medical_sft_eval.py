...
Supported training styles (--training):
  huatuo    – huatuo_stage1/qwen25_full_trainer_eval.py
              (hardcoded "Thinking" / "Final Response")
  medreason – medreason/qwen25_medreason_full_trainer_v2.py
              (same defaults, but titles are configurable at training time)
  custom    – fully manual: supply --thinking-section-title / --final-section-title

To add a new dataset: define adapter functions and register a DatasetAdapter in ADAPTERS.
To add a new training style: add its title defaults to TRAINING_STYLES.
"""
import argparse
import json
import re
...
class DatasetAdapter:
    ...
    # (norm, thinking_title, final_title) -> prompt string shown to the model
    build_prompt: Callable[[dict, str, str], str]

    # (output_text, norm, final_section_re) -> (pred|None, parse_source_str)
    # Multi-step: final section → explicit patterns → option text → single token
    parse: Callable[[str, dict, re.Pattern], tuple[str | None, str]]

    # (norm, draft_response) -> fallback extraction prompt string
    # Used when primary parse returns None.
...
def _mcq4_build_prompt(norm: dict, thinking_title: str, final_title: str) -> str:
...
def _mcq4_parse(text: str, norm: dict, final_section_re: re.Pattern) -> tuple[str | None, str]:
    cleaned = text.replace("<think>", " ").replace("</think>", " ").strip()
    final = _extract_final_section(cleaned, final_section_re)

    # 1. explicit patterns – prefer final section, fallback to full text
    for scope, src in [(final, "final_section"), (cleaned, "full_text")]:
    ...
    # 3. single letter in final section
    if final:
        letters = _MCQ4_LETTER_RE.findall(final.upper())
        if len(letters) == 1:
            return letters[0].upper(), "final_section:single_letter"
...
def _mcq10_build_prompt(norm: dict, thinking_title: str, final_title: str) -> str:
    # The question already has the choices embedded; listing them again wastes context.
    return (
        "You are answering a medical multiple-choice question.\n"
        "Respond in exactly this format:\n"
        f"## {thinking_title}\n"
...
def _mcq10_parse(text: str, norm: dict, final_section_re: re.Pattern) -> tuple[str | None, str]:
    cleaned = text.replace("<think>", " ").replace("</think>", " ").strip()
    final = _extract_final_section(cleaned, final_section_re)
    ...
    # 2. option text matching (options dict available even though question embeds them)
    if final:
        pred = _mcq4_parse_by_option_text(final, norm["options"])
...
def _pubmedqa_build_prompt(norm: dict, thinking_title: str, final_title: str) -> str:
    return (
...
def _pubmedqa_parse(text: str, norm: dict, final_section_re: re.Pattern) -> tuple[str | None, str]:
    cleaned = text.replace("<think>", " ").replace("</think>", " ").strip()
    final = _extract_final_section(cleaned, final_section_re)
    ...
    # 2. single word in final section
    if final:
        words = _PUBMED_RE.findall(final.lower())
        if len(words) == 1:
            return words[0].lower(), "final_section:single_word"
...
# Training style presets: map --training → default section titles.
# These can always be overridden with --thinking-section-title / --final-section-title.
TRAINING_STYLES: dict[str, dict[str, str]] = {
    "huatuo":    {"thinking_title": "Thinking", "final_title": "Final Response"},
    "medreason": {"thinking_title": "Thinking", "final_title": "Final Response"},
    "custom":    {"thinking_title": "Thinking", "final_title": "Final Response"},
}
...
def _force_extract(
    *,
    ...
    pred, _ = adapter.parse(decoded, norm, final_section_re)
    return pred, decoded
...
def run_benchmark(
    *,
    ...
    for i in progress:
        ...
        prompts = [adapter.build_prompt(r, thinking_title, final_title) for r in rows]
        ...
        for idx, norm in enumerate(rows):
            ...
            pred, parse_source = adapter.parse(decoded, norm, final_section_re)
            fallback_used = False
    ...
    result = {
        ...
        "thinking_section_title": thinking_title,
        ...
        "expected_output_format": f"## {thinking_title} / ## {final_title} / The answer is <label>",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help="Override thinking section header (training style default used if empty).",
    )
    parser.add_argument(
        "--final-section-title",
        default="",
        help="Override final answer section header (training style default used if empty).",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    thinking_title = args.thinking_section_title or style_defaults["thinking_title"]
    final_title = args.final_section_title or style_defaults["final_title"]
    ...
    print(
        ...
        f"  sections : ## {thinking_title} / ## {final_title}\n"
        f"  output   : {output_dir}\n"
    )
