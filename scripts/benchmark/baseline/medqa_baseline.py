...
def run_model(
    ...
    for i in progress:
        ...
        prompts = [
            make_prompt(row["question"], row["options"])
            for row in batch
        ]
        texts = [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run MedQA baseline for local models.")
    ...
    parser.add_argument("--output-dir", default="results/medqa_baseline")
